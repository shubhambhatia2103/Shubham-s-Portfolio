...
const inputClasses =
  "mt-1 block w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900/50 text-slate-900 dark:text-white shadow-sm px-3 py-2 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 focus:outline-none transition-colors duration-200";
      ...
      <Reveal delay={0.1}>
        <form onSubmit={handleSubmit} className="mt-6 space-y-4 font-lato lg:mt-10 max-w-2xl mx-auto">
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              className="inline-flex items-center px-5 py-2.5 rounded-lg text-sm font-semibold text-white bg-gradient-to-r from-indigo-500 to-sky-400 hover:from-indigo-600 hover:to-sky-500 shadow-sm shadow-indigo-500/30 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
            >
              Send Message
            </button>
          </div>
          ...
          {formStatus && (
            <div
              className={`text-center text-sm font-medium ${
                formStatus.includes("Oops") ? "text-red-500" : "text-emerald-500"
              }`}
            >
              {formStatus}
            </div>
